...
The whole ruleset lives in tables named `symbivpn*` and is applied atomically
...
class GatewayRules:
    ...
    #: Every other UDP port SymbiVPN binds -- the cluster listener, the time
...
# Generated by SymbiVPN. Do not edit; regenerated whenever the uplink changes.
...
{dashboard_rules}
        ...
        # Nothing SymbiVPN opens may answer the network the laptop has
...
def apply_rules(rules: GatewayRules) -> None:
    """Install the ruleset, replacing any previous SymbiVPN tables."""
...
def teardown() -> None:
    """Remove SymbiVPN's tables, leaving every other rule untouched."""
    if not nft_available():
        return
...
def describe() -> str:
    """The live ruleset, for the dashboard and for `symbivpn status`."""
    if not nft_available():
    ...
    for family, table in (("ip", NAT_TABLE), ("inet", FILTER_TABLE)):
        ...
        if output:
            parts.append(output)
    return "\n".join(parts) or "no SymbiVPN rules are loaded"
